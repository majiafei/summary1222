...
/**
 * @ClassName: DBinHandler
 * @Auther: admin
 * @Date: 2019/11/21 17:20
 * @Description:
 */
public class DBinHandler extends AbstractBinHandler {

    public DBinHandler() {
    }

    public DBinHandler(BinService binService) {
        super(binService);
    }
    ...
    public Bin doGetBin(String volume) {
        String sql = "from "+ Bin.class.getName() + " WHERE is_lock = 0 AND BinName NOT LIKE 'E44%' AND BinName NOT LIKE 'CM%' AND Inv_Material_ID is null AND" +
                " BinName LIKE 'D%' AND volume = '" + volume + "' ORDER BY binName";
